...
void fit_R1R2_pres()
{
	...
	sprintf(peakfile,"/home/kkw/DAQ/ANA500/script/His_Fit_CODE/peak_R%03d/peak_R1R2.dat",runnum);
...
//	m3fitpara ->Branch("pcount_er",&m3pcnter,"m3pcnter/D");
	...
	sprintf(res2file,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/fit_R1R2_%s.C",runnumber3,binnum,runnumber6);
	sprintf(res3file,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/fit_R1R2_%s.root",runnumber3,binnum,runnumber6);
	...
	*/
	...
	his_temp = (TH1D*)hf->Get("mul2_sum_1dim");
	...
	for(int i = 0;i<(line_max1-1);i++){
		...
		if(runnum==151&&i==1){m1fit->FixParameter(1,583.5);}
...
*/
		...
		if(i==0){his_temp->Fit("m1fit","RQ+");}
		if(i==1){his_temp->Fit("m1fit","RQ+");}
/*
