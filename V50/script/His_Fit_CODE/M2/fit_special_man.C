...
//	m3fitpara ->Branch("pcount_er",&m3pcnter,"m3pcnter/D");
	...
	int hist_check1;
	cout<< "single-hit[1] or double-hit[2] :";
	scanf("%i",&hist_check1);

	char hist_check2[258];
	cout<< "[EC] or [BM] : ";
	scanf("%s",hist_check2);

	char hist_check3[258];
	cout<< "[E1] or [E2] : ";
	scanf("%s",hist_check3);

	cout<<"test"<<endl;
	char hist_check_fin[258];
	sprintf(hist_check_fin,"%s_%s_1dim",hist_check2, hist_check3);

	sprintf(hisfile,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/his_%s_M%i.root",runnumber3,binnum,runnumber6,hist_check1);
