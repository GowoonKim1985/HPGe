...
void fit_E2()
{
	...
	char hist_check1[238];
	cout<<"[EC] or [BM] :";
	scanf("%s",hist_check1);

	sprintf(peakfile,"/home/kkw/DAQ/ANA500/script/His_Fit_CODE/peak_R%03d/peak_E2_%s.dat",runnum,hist_check1);
...
//	m1fit->SetParNames("Area","Mean","Sigma","Flat");
	TF1 * m1fit = new TF1("m1fit","([0]/(sqrt(2*TMath::Pi())*[2])*exp(-((x-[1])*(x-[1]))/2/[2]/[2]))+[3]+[4]*x");
	m1fit->SetParNames("Area","Mean","Sigma","p0","p1");
...
*/
	...
	double m1fitpa1, m1fitpa2, m1fitpa3, m1fitpa4, m1fitpa5, m1fiter1, m1fiter2, m1fiter3, m1fiter4, m1fiter5;
	...
	double fixpa[4];//p0, p1, efit1, efit2
	...
	m1fitpara ->Branch("p0",&m1fitpa4,"m1fitpa4/D");
	m1fitpara ->Branch("p1",&m1fitpa5,"m1fitpa5/D");
	...
	m1fitpara ->Branch("p0_er",&m1fiter4,"m1fitper4/D");
	m1fitpara ->Branch("p1_er",&m1fiter4,"m1fitper5/D");
...
//	m3fitpara ->Branch("pcount_er",&m3pcnter,"m3pcnter/D");
	...
	sprintf(res2file,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/fit_%s_E2_%s.C",runnumber3,binnum,hist_check1,runnumber6);
	sprintf(res3file,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/fit_%s_E2_%s.root",runnumber3,binnum,hist_check1,runnumber6);
	...
	*/
	...
	char hist_check_fin[238];
	sprintf(hist_check_fin,"%s_E2_1dim",hist_check1);
	TFile *hf=new TFile(hisfile);
	TH1D * his_temp = new TH1D("his_temp","",bin,0,4000);
//	TH1D * his_full = new TH1D("his_full","",bin,0,4000);
	his_temp = (TH1D*)hf->Get(hist_check_fin);
//	his_full = (TH1D*)hf->Get("mul2_sum_1dim");
...
//	his_full->SetName("fhis_full");
	...
	for(int i = 0;i<(line_max1-1);i++){
		...
		his_temp->Fit("pfit","RQ0+");
		fixpa[0] = pfit->GetParameter(0);
		fixpa[1] = pfit->GetParameter(1);
		his_temp->Fit("efit","RQ0+");
		fixpa[2] = efit->GetParameter(0);
		fixpa[3] = efit->GetParameter(1);
		//m2fit->SetRange((maxx-10),(maxx+10));
		...
		m1fit->ReleaseParameter(4);
		//m1fit->SetParameters(agfitpa1,agfitpa2,agfitpa3,bg);
		m1fit->SetParameters(agfitpa1,maxx,agfitpa3,fixpa[0],fixpa[1]);
...
*/
		...
		m2fit->SetParameters(agfitpa1,maxx,agfitpa3,fixpa[2],fixpa[3]);
		m2fit->FixParameter(3,fixpa[2]);
		m2fit->FixParameter(4,fixpa[3]);
