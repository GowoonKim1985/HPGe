...
//	double peak[N] = {609.321, 768.36, 911.204, 1120.294, 1460.82, 1553.77};	
	double peak[N] = {1001.04, 1553.77};	
	...
	//run650 run2
	  ...
	  sprintf(resfile,"%s/result/Bin%i/res_000650.v1.root", workdir.c_str(), binnum);
...
//	his_temp = (TH1D*)hf->Get("his_tot3");//12det(4,10)
	...
	for(int i = 0;i<N;i++){
		...
		fit->SetRange((maxx-fit_range),(maxx+fit_range));
...
//		p0fit->SetRange((maxx-30),(maxx+30));
		...
		fit->SetParLimits(2,sigma_cal*0.8, sigma_cal*1.2);		
...
//			c2->cd();
	...
	his_temp->SetYTitle(Form("Counts/%.2f kev",bw));
