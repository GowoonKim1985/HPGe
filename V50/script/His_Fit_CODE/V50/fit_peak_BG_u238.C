...
//	scanf("%s", nuname);
	...
	*/
	...
	if(bgnum==2){
	  sprintf(hisfile,"%s/result/Bin%i/his_000657.v1.root", workdir.c_str(), binnum);
	  sprintf(res2file,"%s/result/Bin%i/fit_000657.v1.238U.C", workdir.c_str(), binnum);
	  sprintf(res3file,"%s/result/Bin%i/fit_000657.v1.238U.root", workdir.c_str(), binnum);
	}
