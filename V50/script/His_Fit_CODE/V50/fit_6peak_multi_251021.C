...
//  string workdir("/data/HPGe/USERS/kkw/DAQ/ANA125");
  ...
  int runnum = 650;
  int expnum=2;
  int version = 2; // 1: v1 cut, 2: v2(nocut)
  ...
  double fit_range = 10;  
  int const N = 6; //peaks
  double peak[N] = {768.36, 772.29, 783.29, 785.4, 785.96, 794.95};
  ...
  if(expnum==2){
    if(version==1){
      sprintf(hisfile,"%s/result/Bin%i/his_000650.v1cut.root", workdir.c_str(), binnum);
      sprintf(resfile,"%s/result/Bin%i/mfit_p6_000650_v1cut.root", workdir.c_str(), binnum);
    }
    if(version==2){
  ...
  //  fit->SetRange(peak[0]-50,peak[5]+50);
    fit->SetRange(peak[0]-fit_range,peak[5]+fit_range);
...
//    fit->SetParameter(6, 24.2*bw);//783.29 kev,v50
    fit->SetParameter(6, 1e-4*bw);//783.29 kev,v50
    ...
    //v1
    //area pre set
    fit->SetParameter(0, 381.8*bw);//768.36kev
    fit->SetParameter(3, 6.61*bw);//772.29kev
//    fit->SetParameter(6, 27.35*bw);//783.29 kev,v50
    fit->SetParameter(6, 1e-4*bw);//783.29 kev,v50
    fit->SetParameter(9, 25.3*bw);//785.4 kev
    fit->SetParameter(12, 106.9*bw);//785.96kev
    fit->SetParameter(15, 18.9*bw);//794.95kev
    
    //area limit
    fit->SetParLimits(0, 244.*bw, 518.75*bw);
    fit->SetParLimits(3, 0., 14.1*bw);
    fit->SetParLimits(6, 0., 25.3*bw);
    fit->SetParLimits(9, 0.*bw, 53.4*bw);
    fit->SetParLimits(12, 62.*bw, 152.22*bw);
    fit->SetParLimits(15, 0., 40.3*bw);
...
// ===== Residual & Pull 캔버스 =====
TCanvas *c2 = new TCanvas("c2","Residual & Pull",1200,600);
c2->Divide(2,1);

int nbins = his_temp->GetNbinsX();

// ---------- [1] Residual ----------
c2->cd(1);
TH1D *h_resid = (TH1D*)his_temp->Clone("h_resid");
h_resid->Reset();
for (int i=1; i<=nbins; i++) {
    double x = his_temp->GetBinCenter(i);
    double y = his_temp->GetBinContent(i);
    double yfit = fit->Eval(x);
    h_resid->SetBinContent(i, y - yfit);
 h_resid->SetBinError(i, sqrt(y));     // Residual의 에러 = sqrt(y)

 }
h_resid->SetTitle("Residual (Data - Fit)");
h_resid->GetXaxis()->SetTitle("Energy [keV]");
h_resid->GetYaxis()->SetTitle("Residual [counts]");
h_resid->GetXaxis()->SetRangeUser(peak[0]-fit_range, peak[5]+fit_range);
h_resid->SetLineColor(kBlue);

h_resid->SetMarkerStyle(20);
h_resid->SetMarkerSize(0.7);
h_resid->SetMarkerColor(kBlue);
...
// ---------- [2] Pull ----------
c2->cd(2);
TH1D *h_pull = (TH1D*)his_temp->Clone("h_pull");
h_pull->Reset();
for (int i=1; i<=nbins; i++) {
    double x = his_temp->GetBinCenter(i);
    double y = his_temp->GetBinContent(i);
    double yfit = fit->Eval(x);
    double err = (y > 0) ? sqrt(y) : 1.0;  // Poisson error
    h_pull->SetBinContent(i, (y - yfit) / err);
h_pull->SetBinError(i, 1.0);          // Pull의 에러 = 1로 고정

 }
h_pull->SetTitle("Pull ((Data - Fit)/#sigma)");
h_pull->GetXaxis()->SetTitle("Energy [keV]");
h_pull->GetYaxis()->SetTitle("Pull");
h_pull->GetXaxis()->SetRangeUser(peak[0]-fit_range, peak[5]+fit_range);
...
c2->Update();
  ...
  sprintf(ytitle,"Counts/%.2f keV", bw);
  cout<<"bw "<<bw<<endl;
...
//  his_temp->GetXaxis()->SetRangeUser(750,815);
  his_temp->GetXaxis()->SetRangeUser(peak[0]-fit_range,peak[5]+fit_range);
