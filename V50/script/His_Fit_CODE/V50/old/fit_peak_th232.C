...
//	scanf("%s", nuname);
	...
	sprintf(peakfile,"%s/script/His_Fit_CODE/peak/peak_232Th.dat", workdir.c_str());
...
/*
	...
	sprintf(res2file,"%s/result/RUN%s/Bin%i/fit_%s.232Th.rrs_on_11det.C", workdir.c_str(), runnumber3,binnum,runnumber6);
	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.232Th.rrs_on_11det.root", workdir.c_str(), runnumber3,binnum,runnumber6);
*/
	sprintf(hisfile,"%s/result/RUN%s/Bin%i/his_%s.root", workdir.c_str(), runnumber3, binnum, runnumber6);
	sprintf(res2file,"%s/result/RUN%s/Bin%i/fit_%s.232Th.C", workdir.c_str(), runnumber3,binnum,runnumber6);
	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.232Th.root", workdir.c_str(), runnumber3,binnum,runnumber6);
