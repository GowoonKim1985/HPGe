...
void fit_6peak_multi_250402()
{
	...
	//		pr1=1.15;		pr2=0.31; //rrs on source.cal.
	//		pr1=1.19;		pr2=0.36; //rrs on self .cal.
	//		pr1=1.14;		pr2=0.; //tot source .cal.
	//		pr1=1.29;		pr2=0.21; //rrs on source .cal. 8000
	//		pr1=1.17;		pr2=0.; //tot source .cal. 8000
	//		pr1=0.2;		pr2=0.3; // v1 new cal
		pr1=0.8;		pr2=0.8; // v1 new cal. run1
...
TF1 *fit = new TF1("fit", formula);
	...
	*/
	/*
	sprintf(hisfile,"%s/result/RUN%s/Bin%i/his_%s_2peak_rrs_on.root", workdir.c_str(), runnumber3, binnum, runnumber6);
	sprintf(resfile,"%s/result/RUN%s/Bin%i/mfit_%s_2peak_rrs_on.root", workdir.c_str(), runnumber3, binnum, runnumber6);
	*/
	...
	//	sprintf(hisfile,"%s/result/RUN%s/Bin%i/his_%s.v1.root", workdir.c_str(), runnumber3, binnum, runnumber6);
	//	sprintf(resfile,"%s/result/RUN%s/Bin%i/mfit_%s.v1.root", workdir.c_str(), runnumber3, binnum, runnumber6);
	sprintf(hisfile,"%s/result/V50/Bin%i/his_run1_tot.root", workdir.c_str(), binnum);
	sprintf(resfile,"%s/result/V50/Bin%i/mfit_run1_tot.root", workdir.c_str(), binnum);
...
//      	his_temp = (TH1D*)hf->Get("his_tot4");//11det rrs_on.
	...
	*/
	...
	//fix peak using ref
	  ...
	  fit->FixParameter(1, peakfix[0]);
	  fit->FixParameter(4, peakfix[1]);
	  fit->FixParameter(7, peakfix[2]);
	  fit->FixParameter(10, peakfix[3]);
	  fit->FixParameter(13, peakfix[4]);
	  fit->FixParameter(16, peakfix[5]);
	his_temp->Fit("fit","RL+");
	*/
...
//set range for peak
	...
	double pc1 = fit->GetParameter(10);
	double pc2 = fit->GetParameter(13);
	cout<<pc1<<endl;
	cout<<pc2<<endl;
	if(pc1>=pc2){
	  cout<<"peak 3 & 4 were not fitted well"<<endl;
	  //	  fit->FixParameter(10, peak[3]);
	  //	  fit->FixParameter(13, peak[4]);
	  fit->FixParameter(10, peakfix[3]);
	  fit->FixParameter(13, peakfix[4]);
	}
	...
	his_temp->Fit("fit","RL+");
	*/
	//	fit->SetParLimits(9, -100,100);
	//	fit->SetParLimits(12, -100,100);
...
//3, 4 peak fix
//	fit->SetParLimits(7, peak[2]-0.2, peak[2]+0.3);
//	fit->SetParLimits(10, peak[3]-0.2, peak[3]+0.3);
//	  fit->FixParameter(13, peakfix[4]);
