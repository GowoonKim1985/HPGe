...
//	scanf("%s", nuname);
	...
	sprintf(peakfile,"./peak_50Vcon.dat", workdir.c_str());
...
//	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.%ito%i.root", workdir.c_str(), runnumber3,binnum,runnumber6,isub,fsub);
	sprintf(hisfile,"%s/result/RUN%s/Bin%i/his_%s.rrs_on_conv.root", workdir.c_str(), runnumber3, binnum, runnumber6);
