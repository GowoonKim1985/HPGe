...
//	his_temp = (TH1D*)hf->Get("his_tot3");//12det(4,10)
	...
	for(int i = 0; i<N; i++){
		...
		//		gf/it->SetRange(his_temp->FindBin(peak[i]-3*sigma_cal[i]),his_temp->FindBin(peak[i]+3*sigma_cal[i]));
		//		agfit->SetRange(his_temp->FindBin(peak[i]-3*sigma_cal[i]),his_temp->FindBin(peak[i]+3*sigma_cal[i]));

		gfit->SetRange(peak[i]-3*sigma_cal[i],peak[i]+3*sigma_cal[i]);
		agfit->SetRange(peak[i]-3*sigma_cal[i],peak[i]+3*sigma_cal[i]);
	...
	//bg line fit
	p1fit->SetRange(peak[0]-50,peak[4]+50);
	...
	cout<<"fit range " <<(his_temp->FindBin(peak[0]-50))<<" - "<<(his_temp->FindBin(peak[4]+50))<<endl;
	for(int i=0; i<(3*N+2); i++){fit->ReleaseParameter(i);}
	
	double pr = 1.0;//peak range limit +/-, keV
	for(int i=0; i<5; i++){
	  ...
	  //	  fit->SetParLimits(i*3+1,his_temp->FindBin(peak[i])-2*sigma_cal[i], his_temp->FindBin(peak[i])+2*sigma_cal[i]);
	  //	  cout<<"peak "<<i<<" par limit "<<his_temp->FindBin(peak[i])-3*sigma_cal[i]<<" - "<<his_temp->FindBin(peak[i])+3*sigma_cal[i]<<endl;

	  fit->SetParLimits(i*3+1,his_temp->FindBin(peak[i]-pr), his_temp->FindBin(peak[i])+pr);
	  cout<<"peak "<<i<<" par limit "<<his_temp->FindBin(peak[i]-pr)<<" - "<<his_temp->FindBin(peak[i]+pr)<<endl;
