...
/*	
	int isub, fsub;
	cout<<"start subrun number : ";
	scanf("%i", &isub);
	cout<<"end subrun number : ";
	scanf("%i", &fsub);
*/
/*
	int chnum;
...
//	scanf("%s", nuname);
	...
	sprintf(peakfile,"%s/script/His_Fit_CODE/peak/peak_50V.dat", workdir.c_str());
...
*/
	...
	//		run = hn;
	//
//	sprintf(hisfile,"%s/result/RUN%s/Bin%i/his_%s.%ito%i.root", workdir.c_str(), runnumber3, binnum, runnumber6,isub,fsub);
//	sprintf(res2file,"%s/result/RUN%s/Bin%i/fit_%s.%ito%i.C", workdir.c_str(), runnumber3,binnum,runnumber6,isub,fsub);
//	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.%ito%i.root", workdir.c_str(), runnumber3,binnum,runnumber6,isub,fsub);
	sprintf(hisfile,"%s/result/RUN%s/hisCAL_%s.rrs_on.root", workdir.c_str(), runnumber3, runnumber6);
	//	sprintf(res2file,"%s/result/RUN%s/Bin%i/fit_%s.rrs_on_11det.C", workdir.c_str(), runnumber3,binnum,runnumber6);
	//	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.rrs_on_11det.root", workdir.c_str(), runnumber3,binnum,runnumber6);
	sprintf(res2file,"%s/result/RUN%s/fit_%s.rrs_on_hisCAL.C", workdir.c_str(), runnumber3,runnumber6);
	sprintf(res3file,"%s/result/RUN%s/fit_%s.rrs_on_hisCAL.root", workdir.c_str(), runnumber3,runnumber6);
...
//	his_temp = (TH1D*)hf->Get("his_tot1");//13det
	TH1D * his_temp = (TH1D*)hf->Get("mgthis");//13det
	//	his_temp = (TH1D*)hf->Get("his_tot4");//11det
	his_temp->Draw();
	...
	for(int i = 0;i<N;i++){
		...
		his_temp->GetXaxis()->SetRange(xbin[i]-1*4,xbin[i]+1*4);
		...
		his_temp->GetXaxis()->SetRange(0,16000);
...
//		m3fit->SetRange((maxx-10),(maxx+10));
		...
		m1fit->SetRange((maxx-24*sigma_cal),(maxx+24*sigma_cal));
		m2fit->SetRange((maxx-24*sigma_cal),(maxx+24*sigma_cal));
...
//		m3fit->SetRange((maxx-24*sigma_cal),(maxx+24*sigma_cal));
		...
		bg1 = 0; bg2 = 0; 
		int bgbin = (((int) sigma_cal*3)+1)*4;//mul=4
...
//		his_temp->Fit("m1fit","R+");
		...
		// positive peak fitting for (783 keV)
		if(peak[i]<784&&peak[i]>783){
			m2fit->SetParLimits(0,0,100); //area limit 0~100 (no peak)
		}
		//fitting
		...
		his_temp->Fit("m3fit","R0+");
		m3fitpa[1] = m3fit->GetParameter(1);
		if(abs(m3fitpa[1]-peak[i])>0.3*(sigma_cal)){
			cout <<"m3 test fitting mean "<<m3fitpa[1]<<" for peak "<<peak[i]<<endl;
			cout<<">> peak fixed at"<<peak[i]<<endl;
			m3fit->FixParameter(1,maxx);
