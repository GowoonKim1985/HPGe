...
//	scanf("%s", nuname);
	...
	sprintf(peakfile,"%s/script/His_Fit_CODE/peak/peak_40K.dat", workdir.c_str());
...
//	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.%ito%i.root", workdir.c_str(), runnumber3,binnum,runnumber6,isub,fsub);
	...
	sprintf(res2file,"%s/result/RUN%s/Bin%i/fit_%s.40K.rrs_on_11det.C", workdir.c_str(), runnumber3,binnum,runnumber6);
	sprintf(res3file,"%s/result/RUN%s/Bin%i/fit_%s.40K.rrs_on_11det.root", workdir.c_str(), runnumber3,binnum,runnumber6);
