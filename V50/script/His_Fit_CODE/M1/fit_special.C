...
Double_t gpart[i];
	...
	for(int j=0; j<i; j++){ 
	gpart1[j] = par[0+(4*j)]/(sqrt(2*TMath::Pi())*par[2+(4*j)]);
	gpart2[j] = exp(-((x-par[1+(4*j)])*(x-par[1+(4*j)]))/2/par[2+(4*j)]/par[2+(4*j)]);
	gpart[i]=gpart1[j]*gpart2[j]+par[3+(4*j)];
	}

Double_t finpart = gpart[0]+gpart[1]+gpart[2]+gpart[3]+gpart[4]+gpart[5];
...
//	double m2fitpa1, m2fitpa2, m2fitpa3, m2fitpa4, m2fitpa5, m2fiter1, m2fiter2, m2fiter3, m2fiter4, m2fiter5;
	...
	double fitpa[peaknum][3], fiter[peaknum];
...
*/
	int fit_type;
	double fitpa1, fitpa2, fitpa3, fitchi2, fiter1;
	TTree *fitpara = new TTree("fitpara","Multi Gaus+pol0 fitting result");
...
//	m3fitpara ->Branch("pcount_er",&m3pcnter,"m3pcnter/D");
	...
	sprintf(res2file,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/fit_special_%s.C",runnumber3,binnum,runnumber6);
	sprintf(res3file,"/home/kkw/DAQ/ANA500/result/RUN%s/Bin%i/fit_special_%s.root",runnumber3,binnum,runnumber6);
...
//		m1sum->SetParameter(19, fixpa[2]);
	...
	his_temp->Fit("pfit","R0Q+");
			...
			for(int i =0; i<peaknum; i++){
			fitpa[i][0] = m1sum->GetParameter(0+(3*i));
			fitpa[i][1] = m1sum->GetParameter(1+(3*i));
			fitpa[i][2] = m1sum->GetParameter(2+(3*i));
			fiter[i] = m1sum->GetParError(0+(3*i));
