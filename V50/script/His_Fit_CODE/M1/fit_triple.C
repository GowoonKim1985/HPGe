...
void fit_triple()
{
	...
	scanf("%i",&binnum);
	char peakfile[256];
	sprintf(peakfile,"/home/kkw/DAQ/ANA500/script/His_Fit_CODE/peak_R%03d/peak_triple.dat",runnum);
...
//	m6fitpara -> Branch("run", &runnum, "run/I");
	...
	TTree *m8fitpara = new TTree("m8fitpara", "gaus+gaus+pol1 fitting result");
//	m8fitpara -> Branch("run", &runnum, "run/I");
	m8fitpara -> Branch("peak", &energy, "energy/D");
	m8fitpara -> Branch("area1", &m8fitpa1, "m8fitpa1/D");
	m8fitpara -> Branch("mean1", &m8fitpa2, "m8fitpa2/D");
	m8fitpara -> Branch("sigma1", &m8fitpa3, "m8fitpa3/D");
	m8fitpara -> Branch("area2", &m8fitpa4, "m8fitpa4/D");
	m8fitpara -> Branch("mean2", &m8fitpa5, "m8fitpa5/D");
	m8fitpara -> Branch("sigma2", &m8fitpa6, "m8fitpa6/D");
	m8fitpara -> Branch("area3", &m8fitpa7, "m8fitpa7/D");
	m8fitpara -> Branch("mean3", &m8fitpa8, "m8fitpa8/D");
	m8fitpara -> Branch("sigma3", &m8fitpa9, "m8fitpa9/D");
	m8fitpara -> Branch("pol0", &m8fitpa10, "m8fitpa10/D");
	m8fitpara -> Branch("pol1", &m8fitpa11, "m8fitpa11/D");
	m8fitpara -> Branch("chi2", &m8fitchi2, "m8fitchi2/D");

	m8fitpara -> Branch("area1_er", &m8fiter1, "m8fiter1/D");
	m8fitpara -> Branch("mean1_er", &m8fiter2, "m8fiter2/D");
	m8fitpara -> Branch("sigma1_er", &m8fiter3, "m8fiter3/D");
	m8fitpara -> Branch("area2_er", &m8fiter4, "m8fiter4/D");
	m8fitpara -> Branch("mean2_er", &m8fiter5, "m8fiter5/D");
	m8fitpara -> Branch("sigma2_er", &m8fiter6, "m8fiter6/D");
	m8fitpara -> Branch("area3_er", &m8fiter7, "m8fiter7/D");
	m8fitpara -> Branch("mean3_er", &m8fiter8, "m8fiter8/D");
	m8fitpara -> Branch("sigma3_er", &m8fiter9, "m8fiter9/D");
	m8fitpara -> Branch("pol0_er", &m8fiter10, "m8fiter10/D");
	m8fitpara -> Branch("pol1_er", &m8fiter11, "m8fiter11/D");
	m8fitpara -> Branch("pcount1", &m8pcnt1, "m8pcnt1/D");
	m8fitpara -> Branch("pcount2", &m8pcnt2, "m8pcnt2/D");
	m8fitpara -> Branch("pcount1_er", &m8pcnter1, "m8pcnter1/D");
	m8fitpara -> Branch("pcount2_er", &m8pcnter2, "m8pcnter2/D");
	...
	sprintf(res2file,"/home/kkw/DAQ/ANA500/result/RUN%s/fit_triple_%s.C",runnumber3,runnumber6);
	sprintf(res3file,"/home/kkw/DAQ/ANA500/result/RUN%s/fit_triple_%s.root",runnumber3,runnumber6);
...
/*		his_temp->GetXaxis()->SetRange((peak[i-1]-1)*mul,(peak[i-1]+1)*mul);
...
*/
		...
		if(m6fitpa3<0.4){
			...
			if(peak[i-2]<1000){m6fit->FixParameter(2,0.8);}
			else if(peak[i-2]<2000){m6fit->FixParameter(2,1.1);}
			else{m6fit->FixParameter(2,1.6);}
			}

		if(m6fitpa6<0.4){
			cout <<"Too small sigma(2) value"<<endl;
			cout<<">> sigma(2) fix"<<endl;
			if(peak[i-1]<1000){m6fit->FixParameter(5,0.8);}
			else if(peak[i-1]<2000){m6fit->FixParameter(5,1.1);}
			else{m6fit->FixParameter(5,1.6);}
			}

		if(m6fitpa9<0.4){
			cout <<"Too small sigma(3) value"<<endl;
			cout<<">> sigma(3) fix"<<endl;
			if(peak[i]<1000){m6fit->FixParameter(8,0.8);}
			else if(peak[i]<2000){m6fit->FixParameter(8,1.1);}
			else{m6fit->FixParameter(8,1.6);}
			}


		if(m6fitpa3>1.0&&peak[i-2]<1000){
			cout <<"Too big sigma(1) value @ <1000 keV"<<endl;
			cout<<">> sigma fix"<<endl;
			m6fit->FixParameter(2,0.8);
		}	

		if(m6fitpa6>1.0&&peak[i-1]<1000){
			cout <<"Too big sigma(2) value @ <1000 keV"<<endl;
			cout<<">> sigma fix"<<endl;
			m6fit->FixParameter(5,0.8);
		}

		if(m6fitpa9>1.0&&peak[i]<1000){
			cout <<"Too big sigma(3) value @ <1000 keV"<<endl;
			cout<<">> sigma fix"<<endl;
			m6fit->FixParameter(8,0.8);
		}	

		if(m6fitpa3>2.0&&peak[i-2]>=1000&&peak[i-2]<2000){
			cout <<"Too big sigma(1) value @ 1000~2000 keV"<<endl;
			cout<<">> sigma(1) fix"<<endl;
			m6fit->FixParameter(2,1.1);
		}

		if(m6fitpa6>2.0&&peak[i-1]>=1000&&peak[i-1]<2000){
			cout <<"Too big sigma value @ 1000~2000 keV"<<endl;
			cout<<">> sigma(2) fix"<<endl;
			m6fit->FixParameter(5,1.1);
		}
		if(m6fitpa9>2.0&&peak[i]>=1000&&peak[i]<2000){
			cout <<"Too big sigma value @ 1000~2000 keV"<<endl;
			cout<<">> sigma(3) fix"<<endl;
			m6fit->FixParameter(8,1.1);
		}
...
//		c1->cd();
		...
		if(m7fitpa3<0.4){
			cout <<"Too small sigma(1) value"<<endl;
			cout<<">> sigma(1) fix"<<endl;
			if(peak[i-2]<1000){m7fit->FixParameter(2,0.8);}
			else if(peak[i-2]<2000){m7fit->FixParameter(2,1.1);}
			else{m7fit->FixParameter(2,1.6);}
			}

		if(m7fitpa6<0.4){
			cout <<"Too small sigma(2) value"<<endl;
			cout<<">> sigma(2) fix"<<endl;
			if(peak[i-1]<1000){m7fit->FixParameter(5,0.8);}
			else if(peak[i-1]<2000){m7fit->FixParameter(5,1.1);}
			else{m7fit->FixParameter(5,1.6);}
			}

		if(m7fitpa9<0.4){
			cout <<"Too small sigma(3) value"<<endl;
			cout<<">> sigma(3) fix"<<endl;
			if(peak[i]<1000){m7fit->FixParameter(8,0.8);}
			else if(peak[i]<2000){m7fit->FixParameter(8,1.1);}
			else{m7fit->FixParameter(8,1.6);}
			}


		if(m7fitpa3>1.0&&peak[i-2]<1000){
			cout <<"Too big sigma(1) value @ <1000 keV"<<endl;
			cout<<">> sigma fix"<<endl;
			m7fit->FixParameter(2,0.8);
		}	

		if(m7fitpa6>1.0&&peak[i-1]<1000){
			cout <<"Too big sigma(2) value @ <1000 keV"<<endl;
			cout<<">> sigma fix"<<endl;
			m7fit->FixParameter(5,0.8);
		}	

		if(m7fitpa9>1.0&&peak[i]<1000){
			cout <<"Too big sigma(3) value @ <1000 keV"<<endl;
			cout<<">> sigma fix"<<endl;
			m7fit->FixParameter(8,0.8);
		}	

		if(m7fitpa3>2.0&&peak[i-2]>=1000&&peak[i-2]<2000){
			cout <<"Too big sigma(1) value @ 1000~2000 keV"<<endl;
			cout<<">> sigma(1) fix"<<endl;
			m7fit->FixParameter(2,1.1);
		}

		if(m7fitpa6>2.0&&peak[i-1]>=1000&&peak[i-1]<2000){
			cout <<"Too big sigma value @ 1000~2000 keV"<<endl;
			cout<<">> sigma(2) fix"<<endl;
			m7fit->FixParameter(5,1.1);
		}

		if(m7fitpa9>2.0&&peak[i]>=1000&&peak[i]<2000){
			cout <<"Too big sigma value @ 1000~2000 keV"<<endl;
			cout<<">> sigma(3) fix"<<endl;
			m7fit->FixParameter(8,1.1);
		}
...
//		cout << "m6fit cpd (peak2) :" << m6cpd2 << " +/- " << m6dcpd2 << endl;
		...
		his_temp2->SetTitle("triple peaks Fitting [Gaus + Exp]");
