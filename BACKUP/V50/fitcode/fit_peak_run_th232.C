...
//	scanf("%s", nuname);
	...
	sprintf(peakfile,"%s/script/His_Fit_CODE/V50/peak/peak_232Th.dat", workdir.c_str());
	...
	while (peakdata2.good()){
        ...
        //General resolution run650 3peak (v1)
	...
	*/
	...
	if(expnum==1){
	  ...
	  sprintf(res2file,"%s/result/V50/Bin%i/fit_000624.v0.rrs_on_232Th.C", workdir.c_str(), binnum);
	  sprintf(res3file,"%s/result/V50/Bin%i/fit_000624_v0.rrs_on_232Th.root", workdir.c_str(), binnum);
	}
	
	if(expnum==2){
	  sprintf(hisfile,"%s/result/V50/Bin%i/his_000650.v1.root", workdir.c_str(), binnum);
	  sprintf(res2file,"%s/result/V50/Bin%i/fit_000650.v1_232Th.C", workdir.c_str(), binnum);
	  sprintf(res3file,"%s/result/V50/Bin%i/fit_000650.v1_232Th.root", workdir.c_str(), binnum);

	  /*
	  sprintf(hisfile,"%s/result/V50/Bin%i/his_000650.v1cut.root", workdir.c_str(), binnum);
	  sprintf(res2file,"%s/result/V50/Bin%i/fit_000650.v1cut_232Th.C", workdir.c_str(), binnum);
	  sprintf(res3file,"%s/result/V50/Bin%i/fit_000650.v1cut_232Th.root", workdir.c_str(), binnum);
	  */
	}
